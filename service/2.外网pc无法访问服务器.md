...
### 2.毕竟关闭防火墙不太安全，有个其他的方法不用关闭防火墙也可以实现。简单说一下就是不关闭防火墙，通过开放对应端口来实现外网的访问。不想多说了，看下面步骤图搞定。注意这里演示选择了全部端口，可以根据需要配置端口。  
...
### 对了，还有一个关键步骤是需要在你的云服务器的管理平台上找到“安全组”进行允许访问端口的配置，看一下有没有允许，没有的话配置一下，比较简单，这里就不赘述了。
